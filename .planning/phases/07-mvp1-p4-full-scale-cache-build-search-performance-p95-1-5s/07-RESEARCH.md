...
Capture a clean baseline at current live scale: cache row count, build time, SQL statement count, number of grade aggregate reads, search count-query time, page-query time, hydration time, p50/p95/max, and query parameters. Keep credentials and raw grade rows out of logs. Compare a warm and a cold run if feasible, and record the exact environment and date. This is a recommended procedure based on the observed code paths, not a measured Phase 7 result. [VERIFIED: `src/easy_a/analytics/queries.py:46-78`; `src/easy_a/api/routes/rankings.py:69-191`; `.planning/phases/03.5-ranking-search-performance-at-full-coverage/03.5-PERF-REPORT.md`]
...
| A3 | [ASSUMED] A live read-only benchmark can use the same representative query mix as the synthetic harness. | Validation Architecture | Miss real usage patterns; record mix and vary broad/filtered, seat/no-seat, each sort. |

## Open Questions

1. What portion of p95 is server execution, network, and Python hydration on the actual hosted project? No live engine was available to this researcher; the executor should measure each before selecting an index. [VERIFIED: environment probe 2026-09-22]
2. Is the 3,783-row cache already complete after Phase 6, and how long does a fresh whole-term rebuild take today? The validator reports search total reconciliation, but it does not time the rebuild. [VERIFIED: `.planning/phases/06-mvp1-p3-all-tampa-section-ingestion-10-3-782/06-03-SUMMARY.md`; `scripts/validate_tampa_ingest.py`]
3. Which exact representative query mix defines the p95 gate? Existing harness cycles sorts, subject filtering, seats-open, easiness floor, limits and offsets; the plan should retain that mix and add clearly labeled live-data runs. [VERIFIED: `scripts/benchmark_rankings_search.py:370-390`]
...
| REQ-PERF-01 | p95 below about 1.5 seconds at real scale | Dated hosted-Supabase live read-only benchmark over 3,783 real sections, with dataset, query mix, p50/p95/max and environment; manual acceptance evidence. [VERIFIED: `.planning/ROADMAP.md:394-416`] |
